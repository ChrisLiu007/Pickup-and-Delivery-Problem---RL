...
def generate_pdp_data(dataset_size, pdp_size):
    ...
    locs = np.random.uniform(size=(dataset_size, pdp_size, 2))  # Node locations

    demand = np.random.randint(1, 10, size=(dataset_size, pdp_size // 2)).repeat(2, axis=1) # Demand, uniform integer 1 ... 9
    demand[:, 1::2] *= -1

    type = np.tile([True, False], (dataset_size, pdp_size // 2)).tolist()

    p_or_d = np.empty((dataset_size, pdp_size, 2))
    p_or_d[:, ::2, :] = locs[:, 1::2, :]
    p_or_d[:, 1::2, :] = locs[:, ::2, :]

    return list(zip(
        np.random.uniform(size=(dataset_size, 2)).tolist(),  # Depot location
        locs.tolist(),  # Node locations
        demand.tolist(),  # Demand, uniform integer 1 ... 9
        np.full(dataset_size, CAPACITIES[pdp_size]).tolist(),  # Capacity, same for whole dataset
        type,
        p_or_d.tolist()
    ))
