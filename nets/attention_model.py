...
class AttentionModel(nn.Module):
    ...
    def _init_embed(self, input):
        ...
        features = ('demand', 'type', 'p_or_d')  # p_or_d

        # DEBUG:
        #print(input)
        #print(input['loc'])
        #print(input['type'][:, :, None])
        #print(input['p_or_d'][:, :, None])
        # DEBUG:

        #print(#torch.cat(
            #(
                #self.init_embed_depot(input['depot'])[:, None, :], # JAKOB: Probably remove this. Not of importance with open-ended routes
    ...
    def _get_log_p(self, fixed, state, normalize=True):
        ...
        glimpse_K, glimpse_V, logit_K = self._get_attention_node_data(fixed, state) # EDIT: Jakob. I try to add "visited_" to context here.
    ...
    def _get_attention_node_data(self, fixed, state): # EDIT JAKOB: This was taken from original Kool implementation with sdvrp.
        # EDIT JAKOB: This could be an easy way to project information of "visited" to the context.

        # Need to provide information of how much each node has already been served
        # Clone demands as they are needed by the backprop whereas they are updated later
        glimpse_key_step, glimpse_val_step, logit_key_step = \
            self.project_node_step(state.visited2[:, :, :, None].clone()).chunk(3, dim=-1)
    ...
    def _make_heads(self, v, num_steps=None):
        ...
        return v.contiguous().view(v.size(0), v.size(1), v.size(2), self.n_heads, -1)\
            .expand(v.size(0), v.size(1), v.size(2), self.n_heads, -1)\
